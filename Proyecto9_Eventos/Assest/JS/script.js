...
const json = cargar();
// esto es pa que carge la lista de datos
try {
    arr = JSON.parse(json);
} catch (error) {
    arr = []
...
function diferenciaFecha(destino) {
    ...
    let dias = Math.ceil(diferencia / (1000 * 3600 * 24)); //ESTO SE UTILIZA PARA ACERCARCE AL DIA MAS CERCANO, milisegundos=> horas => dias
    return dias;
}
...
function mostrarEventos() {
    const eventosHTML = eventos.map((evento) =>{
        return`
         <div class="evento">
          ...
          <div class="nombreEven">${evento.nombre}</div>
          <div class="fechaEven">${evento.fecha}</div>
          <div class="acciones">
    ...
    document.querySelectorAll('.eliminar').forEach(button => {
        button.addEventListener("click",e => {
            ...
            guardar(JSON.stringify(eventos));//esta linea es para que muestre los eventos, aunque se recargue la pagina
...
function cargar() {
    return localStorage.getItem("lista");// getItem es para traer los datos
}